...
function make_pip_repo()
{
    ...
    rm -rf pip-ops; mkdir -p pip-ops

    tar -zxvf openstack_pip.tar.gz; cp -f openstack_pip/* pip-ops/

    cp -f pip/* pip-ops/

    tar -zcvf pip-ops.tar.gz ./pip-ops; cd -
...
function make_jhenv_repo()
{
    ...
    if [[ -d ${COMPASS_PATH}/repo/jhenv_template/$env_os_name ]]; then
        ...
        cp -rf ${COMPASS_PATH}/repo/jhenv_template/$env_os_name/$jh_os/${jh_env_dockerfile} ${COMPASS_PATH}/work/repo/jhenv_template
