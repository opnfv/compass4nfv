...
{
    cd $CACHE_DIR/$1/kvmfornfv/kernel
...
function make_qemu()
{
    mkdir -p $CACHE_DIR/$1/kvmfornfv/qemu/build
    cd $CACHE_DIR/$1/kvmfornfv/qemu/build
...
function make_kvmfornfv()
{
    make_kernel $1
    make_qemu $1
    tar czvf $CACHE_DIR/$1/app_packages/kvmfornfv_$(date +%F_%R).tar.gz $CACHE_DIR/$1/kvmfornfv
}
