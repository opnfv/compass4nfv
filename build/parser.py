...
def get_from_cache(cache, package):
    ...
    if os.path.exists(localmd5file):
        ...
        print "Compare local md5 %s and remote md5 %s" %(localmd5sum, remotemd5sum)
...
def get_from_docker(cache, package):
    ...
    os.system(cmd)
    cmd = "sudo docker save "+package.get("url")+" -o "+cache+"/"+package.get("name")+".tar"
    os.system(cmd)

def get_from_curl(cache, package):
    cmd = "curl --connect-timeout 10 -o " + cache + "/" +package.get("name") + " " + package.get("url")
    print cmd
    os.system(cmd)
